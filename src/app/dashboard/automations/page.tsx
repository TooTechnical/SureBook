...
import { optionalSchemaQuery } from "@/lib/optional-schema";
...
export default async function AutomationsPage() {
  ...
  const savedQuery: Promise<(typeof automationDefinitions.$inferSelect)[]> = db.select().from(automationDefinitions).where(eq(automationDefinitions.salonId, session.salonId));
  const reportsQuery: Promise<(typeof weeklyReportPreferences.$inferSelect)[]> = db.select().from(weeklyReportPreferences).where(eq(weeklyReportPreferences.salonId, session.salonId)).limit(1);
  const [savedResult, reportsResult] = await Promise.all([
    optionalSchemaQuery(savedQuery, []),
    optionalSchemaQuery(reportsQuery, []),
  ]);
  const saved = savedResult.data;
  const reports = reportsResult.data[0];
  const schemaAvailable = savedResult.schemaAvailable && reportsResult.schemaAvailable;
  ...
  return <><div><span className="badge">Owner-controlled lifecycle</span><h1>Automations</h1><p style={{ color: "var(--muted)", maxWidth: 760 }}>Configure transactional and consent-aware messages. SureBook never enables or sends a workflow without business configuration.</p></div>
    {!schemaAvailable && <section className="card" role="alert" style={{ padding: 20, borderColor: "#d9b84d", background: "#fff9e8" }}><h2>Database update required</h2><p>Automations cannot be configured until an operator runs <code>npx drizzle-kit push</code> against this environment.</p></section>}
