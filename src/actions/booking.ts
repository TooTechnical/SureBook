...
import { discountCodes, referrals } from "@/db/marketing-schema";
...
export async function createBookingIntent(input: z.infer<typeof inputSchema>) {
  ...
  const referralCode = (await cookies()).get("surebook_referral")?.value;
  const referral = referralCode ? await db.query.referrals.findFirst({ where: and(eq(referrals.code, referralCode), eq(referrals.salonId, salon.id), or(eq(referrals.status, "issued"), eq(referrals.status, "visited"))) }) : null;
  ...
  const booking = await db.transaction(async (tx) => {
    ...
    const notes = [discount ? `Discount ${discount.code}: €${(discountAmountCents / 100).toFixed(2)} credit against service total.` : null, referral ? `Referral ${referral.code} attached.` : null].filter(Boolean).join("\n") || null;
    const [created] = await tx.insert(bookings).values({ salonId: salon.id, customerId: customer.id, staffId: teamMember.id, serviceId: service.id, startsAt, endsAt, depositCents, status: depositCents > 0 ? "pending_payment" : "confirmed", paymentStatus: depositCents > 0 ? "pending" : "not_required", internalNotes: notes }).returning();
    if (discount) {
    ...
    if (referral) await tx.update(referrals).set({ referredCustomerId: customer.id, status: "booked" }).where(eq(referrals.id, referral.id));
    return created;
  });
