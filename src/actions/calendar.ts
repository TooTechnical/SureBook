...
export async function createCalendarBlockAction(formData: FormData) {
  ...
  const input = z.object({
    ...
    blockType: z.enum(["blocked", "holiday", "lunch", "personal"]),
  ...
  if (!(endsAt > startsAt)) throw new Error("The block must end after it starts.");
  await db.insert(calendarBlocks).values({
    ...
    startsAt,
    endsAt,
    allDay: input.allDay === "on",
    recurrenceRule: input.recurrence === "none" ? null : `FREQ=${input.recurrence.toUpperCase()}`,
...
export async function deleteCalendarBlockAction(formData: FormData) {
  const session = await requireSession();
  const id = z.string().uuid().parse(formData.get("blockId"));
...
export async function createRecurringAppointmentsAction(formData: FormData) {
  const session = await requireSession();
  const input = z.object({
    customerId: z.string().uuid(), staffId: z.string().uuid(), serviceId: z.string().uuid(),
    firstStartsAt: z.string().min(1), frequency: z.enum(["weekly", "fortnightly", "monthly"]),
  ...
  for (let index = 0; index < input.occurrenceCount; index += 1) {
    starts.push(input.frequency === "weekly" ? addWeeks(firstStartsAt, index) : input.frequency === "fortnightly" ? addWeeks(firstStartsAt, index * 2) : addMonths(firstStartsAt, index));
  }
  for (const startsAt of starts) {
    const endsAt = new Date(startsAt.getTime() + service.durationMinutes * 60_000);
    await assertSlotAvailable({ salonId: session.salonId, staffId: input.staffId, startsAt, endsAt });
  }
