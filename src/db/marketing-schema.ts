...
export const salonsMarketingRelations = relations(salons, ({ many }) => ({ aiGenerations: many(aiGenerations), discountCodes: many(discountCodes), referralCampaigns: many(referralCampaigns), giftVouchers: many(giftVouchers), memberships: many(memberships), servicePackages: many(servicePackages) }));
