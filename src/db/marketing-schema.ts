...
export const referralCampaigns = pgTable("referral_campaigns", {
  ...
  advocateRewardCents: integer("advocate_reward_cents").notNull(),
  friendRewardCents: integer("friend_reward_cents").notNull(),
  terms: text("terms"),
  active: boolean("active").notNull().default(true),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
});
...
export const referrals = pgTable("referrals", {
  ...
  salonId: uuid("salon_id").references(() => salons.id, { onDelete: "cascade" }).notNull(),
  referrerCustomerId: uuid("referrer_customer_id").references(() => customers.id, { onDelete: "set null" }),
...
export const giftVouchers = pgTable("gift_vouchers", {
  ...
  salonId: uuid("salon_id").references(() => salons.id, { onDelete: "cascade" }).notNull(),
  code: varchar("code", { length: 40 }).notNull().unique(),
  purchaserName: varchar("purchaser_name", { length: 160 }),
...
export const membershipSubscriptions = pgTable("membership_subscriptions", {
  ...
  status: purchaseStatus("status").notNull().default("active"),
  visitsRemaining: integer("visits_remaining").notNull().default(0),
...
export const packagePurchases = pgTable("package_purchases", {
  id: uuid("id").defaultRandom().primaryKey(),
  packageId: uuid("package_id").references(() => servicePackages.id, { onDelete: "restrict" }).notNull(),
  salonId: uuid("salon_id").references(() => salons.id, { onDelete: "cascade" }).notNull(),
  customerId: uuid("customer_id").references(() => customers.id, { onDelete: "restrict" }).notNull(),
...
export const salonsMarketingRelations = relations(salons, ({ many }) => ({
  aiGenerations: many(aiGenerations), discountCodes: many(discountCodes), referralCampaigns: many(referralCampaigns),
  giftVouchers: many(giftVouchers), memberships: many(memberships), servicePackages: many(servicePackages),
}));
